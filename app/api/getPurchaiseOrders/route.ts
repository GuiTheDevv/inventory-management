...
export async function POST(req: NextRequest) {
  ...
  const getPurchaiseOrders = async () => {
    try {
      const url = `https://www.zohoapis.com/inventory/v1/purchaseorders?organization_id=837554536`;
  ...
  const purchaiseOrders = await getPurchaiseOrders();

  return NextResponse.json({ data: purchaiseOrders.purchaseorders });
}
